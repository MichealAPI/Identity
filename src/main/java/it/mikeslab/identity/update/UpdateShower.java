...
public class UpdateShower {
    ...
    private InventoryGui getInventoryByUpdateVersion(String updateVersion) {
        return Inventories.inventories.get("update-" + updateVersion);
    }
    ...
    public ItemStack getUpdateItemStack(Update update) {
        switch (update.getUpdateType()) {
            case BUGFIX:
                return CustomHeads.getCustomHead(bugfix_headValue, MiniMessage.miniMessage().deserialize("<color:#FF3131>Bug-Fix").decoration(TextDecoration.ITALIC, false), Collections.singletonList(MiniMessage.miniMessage().deserialize("<color:#FF3131><bold>*</bold> <gray>" + update.getShortDescription()).decoration(TextDecoration.ITALIC, false)));
            case IMPORTANT:
                return CustomHeads.getCustomHead(important_headValue, MiniMessage.miniMessage().deserialize("<color:#D98A00>Fix").decoration(TextDecoration.ITALIC, false), Collections.singletonList(MiniMessage.miniMessage().deserialize("<color:#D98A00><bold>*</bold> <gray>" + update.getShortDescription()).decoration(TextDecoration.ITALIC, false)));
            case FEATURE:
                return CustomHeads.getCustomHead(feature_headValue, MiniMessage.miniMessage().deserialize("<color:#18FF00>Feature").decoration(TextDecoration.ITALIC, false), Collections.singletonList(MiniMessage.miniMessage().deserialize("<color:#18FF00><bold>+</bold> <gray>" + update.getShortDescription()).decoration(TextDecoration.ITALIC, false)));
        }
