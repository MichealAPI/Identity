...
import it.mikeslab.identity.utils.inventory.GenderCommands;
    ...
    public void openNextInventory(Player player, Identity plugin, PersonUtil personUtil, Inventories inventories, PostProcessCommands postProcessCommands, CustomConfigsInit customConfigsInit, boolean setup) {
        if (setup) {
            ...
            if (nxtInventory == null) {
                ...
                switch (nxtInventory) {
                    case "name": {
                        inventories.openGUI(InventoryType.NAME, Settings.INVENTORY_NAME_TYPE, plugin, player, customConfigsInit, personUtil, postProcessCommands, setup);
                    }
                    case "gender": {
                        inventories.openGUI(InventoryType.GENDER, Settings.InventoryType.CHEST, plugin, player, customConfigsInit, personUtil, postProcessCommands, setup);
                    }
                    case "age": {
                        inventories.openGUI(InventoryType.AGE, Settings.INVENTORY_AGE_TYPE, plugin, player, customConfigsInit, personUtil, postProcessCommands, setup);
                    }
                }
